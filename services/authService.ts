...
export class AuthService {
  async requestOtp(data: any) {
    ...
    switch (purpose) {
      ...
      case 'forgot-password':
        if (!existingAccount) throw new Error('No account found with this email');
        break;
      default:
  ...
  async verifyOtp(data: any) {
    ...
    switch (purpose) {
      ...
      case 'forgot-password': {
        ...
        const resetToken = jwt.sign({ id: account._id, email: account.email, purpose: 'resetPassword' }, JWT_SECRET as string, { expiresIn: '15m' });
  ...
  async register(data: any) {
    ...
    const existingAccount = await accountRepository.findByEmailOrUsername(email, username);
    if (existingAccount) {
    ...
    await accountRepository.pushRefreshToken(savedAccount._id.toString(), refreshToken);
  ...
  async login(data: any) {
    ...
    if (!isMatch) {
      await accountRepository.incrementFailedLogins(account, MAX_LOGIN_ATTEMPTS, LOCK_TIME);
      throw new Error('Invalid username or password');
    }
    ...
    await accountRepository.pushRefreshToken(account._id.toString(), refreshToken);
  ...
  async refreshAccessToken(data: any) {
    ...
    const accountWithTokens = await accountRepository.findForAuth(decoded.username);
    
    if (!accountWithTokens || !accountWithTokens.refreshTokens.includes(refreshToken)) {
    ...
    const payload = { id: accountWithTokens._id, username: accountWithTokens.username, role: accountWithTokens.role };
